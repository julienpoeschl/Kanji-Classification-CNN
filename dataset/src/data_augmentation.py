...
class DataAugmenter:
    ...
    def _scale(self, image: np.ndarray) -> np.ndarray:
        ...
        # Zoom the image
        zoomed = ndimage.zoom(image, scale, order=1)
    ...
    def _shift(self, image: np.ndarray) -> np.ndarray:
        ...
        return ndimage.shift(image, (shift_h, shift_w), mode='constant', cval=0)
    ...
    def _elastic_deform(self, image: np.ndarray) -> np.ndarray:
        ...
        return ndimage.map_coordinates(image, coords, order=1, mode='constant', cval=0)
    ...
    def augment_single(self, image: np.ndarray) -> np.ndarray:
        ...
        if self._settings.elastic_enabled:
            result = self._elastic_deform(result)
